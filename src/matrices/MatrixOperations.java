...
public class MatrixOperations implements IMatrixOperations {
    ...
    @Override
    public Number[][] multiplyMatrices(Number[][] matrixA, Number[][] matrixB) throws InvalidSizeExcpetion {
        ...
        if (lines != columns) {
            throw new InvalidSizeExcpetion("Matrices don't have same size");
        } else {
            ...
            for (int i = 0; i < lines; i++) {
                ...
                for (int j = 0; j < columns; j++) {
                    ...
                    for (int k = 0; k < columns; k++) {
                        mult = NumberOperations.multiplication(matrixA[i][k], matrixB[k][j]);
                        sum = NumberOperations.summation(sum, mult);
                    }
                    result[i][j] = sum;
                }
            }
